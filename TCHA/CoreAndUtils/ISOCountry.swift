...
class IsoCountries {
    ...
    static func getCountry(from isoCode:String) -> (alpha3: String, name: String)? {
        return IsoCountries.allLocalizedISO3Countries().filter{ $0.alpha3 == isoCode }.first
    }
    
    static func allLocalizedISO3Countries(first iso3Country: String? = nil) -> Array<(alpha3: String, name: String)> {
        let countryList = Locale.isoRegionCodes.compactMap { (alpha2: $0, name:Locale.current.localizedString(forRegionCode: $0)) }
        let mapiso23 = IsoCountries.iso2ToIso3Map()
        var countries =  countryList.compactMap { (alpha2: String, name: String?) -> (alpha3: String, name: String)? in
            guard let alpha3 = mapiso23[alpha2], let _localizedName = name else { return nil }
            return (alpha3: alpha3, name: _localizedName)
        }.sorted(by: { $0.name < $1.name })
        
        if let countryToDrop = iso3Country, let dropped = countries.first(where: { $0.alpha3 == countryToDrop }) {
            countries.removeAll(where: { $0.alpha3 == countryToDrop })
            countries.insert(dropped, at: 0)
        }
    ...
    static func allCountries(first isoCountry: String? = nil) -> Array<IsoCountryInfo> {
        
        var countries = IsoCountries.countriesInfo
        
        if let countryToDrop = isoCountry, let dropped = countries.first(where: { $0.alpha3 == countryToDrop }) {
            countries.removeAll(where: { $0.alpha3 == countryToDrop })
            countries.insert(dropped, at: 0)
        }
    ...
    static func iso2ToIso3Map() -> [String : String] {
        var iso23 : [String: String] = [:]
        for isocountry in IsoCountries.allCountries() {
            iso23[isocountry.alpha2] = isocountry.alpha3
        }
        return iso23
    }
...
fileprivate extension IsoCountries {
    
    
    static var countriesInfo: Array<IsoCountryInfo> {
        return IsoCountries.countriesISOCodes.map { tuple in
            IsoCountryInfo(name: tuple.0,
                           numeric: tuple.3,
                           alpha2: tuple.1,
                           alpha3: tuple.2)
        }
    ...
    static let countriesISOCodes: Array<(String, String, String, String, String, String)> = [
